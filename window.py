...
# Function after Predict Button is Clicked
def btn_clicked():
    global input_actor, input_director, actor_window, director_window
    mov = input_mov.get()
    year = input_year.get()
    ...
    if '.' in result:
        ...
        input_actor.delete(0, 'end')  # Clear the entry
        input_director.delete(0, 'end')  # Clear the entry
        actor_window = my_canvas.create_window(34, 220, anchor="nw", window=input_actor)
        director_window = my_canvas.create_window(34, 300, anchor="nw", window=input_director)
def home():
    ...
    my_canvas.create_text(172, 120, text="Enter Year of Release :", font=("HP Simplified Hans", 20), fill="white")
    my_canvas.create_text(160, 195, text="Enter Actor Name :", font=("HP Simplified Hans", 20), fill="white")
    my_canvas.create_text(169, 275, text="Enter Director Name :", font=("HP Simplified Hans", 20), fill="white")

    # Define entry boxes
    global input_mov, input_year, input_actor, input_director
    ...
    input_actor = Entry(window, font=("Segoe UI", 18), width=45, bg="#ade5f1", bd=0)
    input_director = Entry(window, font=("Segoe UI", 18), width=45, bg="#ade5f1", bd=0)

    # Add the entry boxes to the canvas
    global mov_window, year_window, actor_window, director_window
